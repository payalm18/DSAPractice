...
namespace FindIndexSumPair
{
    ...
    {
        public static int[] FindIndexSumPair(int[] nums, int target)
        {
            ...
            for (int i = 0; i < numsCopy.Length; i++)
            {
                ...
                if (numsCopy[i] == numsCopy[i + 1] && i < numsCopy.Length)
                {
                    int addNums = nums[i] + nums[i + 1];
                    if (target == addNums)
                    {
                        arrayIndices[0] = Array.IndexOf(nums, numsCopy[i]);
                        arrayIndices[1] = (Array.IndexOf(nums, numsCopy[i + 1]) + 1);
                    }
                    return arrayIndices;
                }
                if (sum.Contains(nums[i]))
                {
                    ...
                    arrayIndices[1] = Array.IndexOf(nums, item);
                    Array.Sort(arrayIndices);
                    return arrayIndices;
                }
                else
                {
                    sum.Add(target - nums[i]);
                }
            }
            arrayIndices[0] = -1;
            arrayIndices[1] = -1;
        ...
        public static int[] FindIndexSumPair1(int[] nums, int target)
        {
            int[] arrayIndices = new int[2];
            int count = 1;
            for (int i = 0; i < nums.Length; i++)
            {
                ...
                for (int j = 0+count; j < nums.Length; j++)
                {
                    int sumPair = nums[i] + nums[j];
                    int c = nums.Count(x => x.Equals(nums[i]));
                    if (target == sumPair)
                    {
                        if (nums[i] == nums[j] && c > 1)
                        {
                            arrayIndices[0] = Array.IndexOf(nums, nums[i]);
                            //arrayIndices[1] = (Array.IndexOf(nums, nums[j])+ counter);
                            arrayIndices[1] = Array.LastIndexOf(nums, nums[j]);
                            return arrayIndices;
                        }
                        if ( nums[i] != nums[j])
                        {
                            arrayIndices[0] = Array.IndexOf(nums, nums[i]);
                            arrayIndices[1] = Array.IndexOf(nums, nums[j]);
                            //counter++;
                            return arrayIndices;
                        }
                    }
        ...
        public static int[] FindIndexSumPair2(int[] nums, int target)           //Optimal solution!
        {
            ...
            for (int i = 0; i<nums.Length; i++)
            {
                ...
                {
                   // var ele = set;
                    arrayIndices[0] = Array.IndexOf(nums, (target - nums[i]));      //To find the index of the pair
                    arrayIndices[1] = Array.LastIndexOf(nums, nums[i]) ;            //To find the index of the number
                    return arrayIndices;
                }
                else
                {
